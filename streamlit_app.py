...
st.set_page_config("Kalman Filter Car Game", layout="wide", initial_sidebar_state="expanded")
...
# --- SIDEBAR widgets using .get() for KeyError avoidance ---
st.sidebar.header("⚙️ Configuration")
st.sidebar.subheader("Initial Car State")
init_pos_x = st.sidebar.slider("Initial Position X (m)", -100.0, 100.0, st.session_state.get('init_pos_x', 0.0), step=1.0, key='init_pos_x')
init_pos_y = st.sidebar.slider("Initial Position Y (m)", -100.0, 100.0, st.session_state.get('init_pos_y', 0.0), step=1.0, key='init_pos_y')
init_vel_x = st.sidebar.slider("Initial Velocity X (m/s)", -20, 20, st.session_state.get('init_vel_x', 5.0), step=0.5, key='init_vel_x')
init_vel_y = st.sidebar.slider("Initial Velocity Y (m/s)", -20, 20, st.session_state.get('init_vel_y', 2.0), step=0.5, key='init_vel_y')

st.sidebar.subheader("Motion Parameters")
dt = st.sidebar.slider("Time Step (seconds)", 0.01, 1.0, st.session_state.get('dt', 0.1), step=0.01, key='dt')
duration = st.sidebar.slider("Simulation Duration (steps)", 10, 500, st.session_state.get('duration', 100), step=10, key='duration')
acceleration_x = st.sidebar.slider("Acceleration X (m/s²)", -5.0, 5.0, st.session_state.get('acceleration_x', 0.0), step=0.1, key='acceleration_x')
acceleration_y = st.sidebar.slider("Acceleration Y (m/s²)", -5.0, 5.0, st.session_state.get('acceleration_y', 0.0), step=0.1, key='acceleration_y')

st.sidebar.subheader("Noise Parameters")
measurement_noise = st.sidebar.slider("Measurement Noise σ (m)", 0.1, 50.0, st.session_state.get('measurement_noise', 5.0), step=0.5, key='measurement_noise')
process_noise_pos = st.sidebar.slider("Process Noise (Position) σ (m²)", 0.001, 1.0, st.session_state.get('process_noise_pos', 0.1), step=0.01, key='process_noise_pos')
process_noise_vel = st.sidebar.slider("Process Noise (Velocity) σ (m²/s²)", 0.001, 1.0, st.session_state.get('process_noise_vel', 0.01), step=0.01, key='process_noise_vel')
...
if 'kf' not in st.session_state or not isinstance(st.session_state.kf, Kalman2DFilter):
    st.session_state.kf = Kalman2DFilter(dt=st.session_state.get('dt', 0.1))
...
if col2.button("🔄 Reset", use_container_width=True):
    ...
    st.session_state.kf = Kalman2DFilter(dt=DEFAULTS['dt'])
    st.session_state.simulation_data = None
    st.session_state.run_simulation = False
    st.experimental_rerun()
...
if col3.button("📊 Example Data", use_container_width=True):
    ...
    st.session_state.kf = Kalman2DFilter(dt=EXAMPLES['dt'])
    st.session_state.simulation_data = run_kalman_simulation(
        ...
        duration=EXAMPLES['duration'], dt=EXAMPLES['dt'],
...
if st.session_state.run_simulation:
    with st.spinner("🔄 Running Kalman filter simulation..."):
        sim_data = run_kalman_simulation(
            st.session_state.kf,
            init_pos_x=st.session_state.get('init_pos_x', 0.0),
            init_pos_y=st.session_state.get('init_pos_y', 0.0),
            init_vel_x=st.session_state.get('init_vel_x', 5.0),
            init_vel_y=st.session_state.get('init_vel_y', 2.0),
            duration=st.session_state.get('duration', 100),
            dt=st.session_state.get('dt', 0.1),
            acceleration_x=st.session_state.get('acceleration_x', 0.0),
            acceleration_y=st.session_state.get('acceleration_y', 0.0),
            measurement_noise=st.session_state.get('measurement_noise', 5.0),
            process_noise_pos=st.session_state.get('process_noise_pos', 0.1),
            process_noise_vel=st.session_state.get('process_noise_vel', 0.01)
        )
...
# (Insert your existing tabs/visualization code block here)
if st.session_state.simulation_data is not None:
    # ... visualization code as in your working example ...
    pass
else:
